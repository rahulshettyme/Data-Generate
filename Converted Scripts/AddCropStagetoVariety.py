def run(data, token, env_config):
    ...
    def _user_run(data, token, env_config):
        ...
        def fetch_crop_stages_master():
            """Step 1 Master Data Fetch: Fetch and cache crop stages master data (Run Once)."""
            if cache['crop_stage_map'] is None:
                if cache['crop_stage_map'] is None:
                    url = f'{base_url}/services/farm/api/crop-stages'
                    print(f'Fetching master crop stages from {url}')
                    try:
                        response = _log_get(url, headers={'Authorization': f'Bearer {token}'})
        ...
        def process_group(item):
            """Processes all rows belonging to a single varietyID."""
            ...
            try:
                response_get = _log_get(variety_url, headers={'Authorization': f'Bearer {token}'})
            ...
            if harvest_days_input is not None and str(harvest_days_input).strip():
                try:
                    ...
                    pass
            variety_payload_template = attribute_utils.add_attributes_to_payload(rows[0], variety_payload_template, env_config, target_key='data')
            update_url = f'{base_url}/services/farm/api/varieties'
            payload = variety_payload_template
            try:
                response_put = _log_put(update_url, headers=auth_headers, data=json.dumps(payload))
        ...
        for item in grouped_data.items():
            group_rows = process_group(item)
            processed_results.append(group_rows)
